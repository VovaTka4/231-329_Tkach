...
ALLOWED_HOSTS = ['*'] # > СПИСОК ХОСТНЕЙМОВ ИЗ ЗАГОЛОВКОВ HTTP-ЗАПРОСОВ, КОТОРЫМ БЭКЕНД НЕ БУДЕТ ОТКАЗЫВАТЬ В ОБРАБОТКЕ
...
DATABASES = {
    'default': {
        ...
        'NAME': 'lab2_db', # <<< Название БД
        'USER': 'django', # <<< Внутенний пользователь БД, нужно создать вручную или скриптом 
        'PASSWORD': 'django_password', # <<< Пароль юзера
        'HOST': 'postgres-service', # <<< тут установлен postgres-service, чтобы сервис бекенда подружился с сервисом постгреса. Изначально было localhost)
        'PORT': 5432, # <<< 
    }
}
